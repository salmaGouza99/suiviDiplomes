...
function BureauOrdre(props) {
    ...
    useEffect(() => {
        ...
        // test if the user is loggedout or he hasn't the role of BureauOrdre, we redirect him to the login page
        if(loggedOut === true || props?.role !== 7) {
            history.push("/");
        // else we show him the content of this page
        } else {
            setNavItems([
                {   // set the navbar of Bureau d'Ordre
                    id: 'Diplômes',
                    children: [
                        { id: 'En attente de réception', icon: <ScheduleIcon />, index: '1'},
                        { id: 'Reçus', icon: <ReceivedIcon />, index: '2'},
                    ],
                },
                {
                    id: 'Profil',
